...
import org.hl7.fhir.r6.model.CapabilityStatement.CapabilityStatementDocumentComponent;
import org.hl7.fhir.r6.model.CapabilityStatement.CapabilityStatementMessagingComponent;
import org.hl7.fhir.r6.model.CapabilityStatement.CapabilityStatementMessagingSupportedMessageComponent;
import org.hl7.fhir.r6.model.CapabilityStatement.CapabilityStatementMessagingEndpointComponent;
import org.hl7.fhir.r6.model.CapabilityStatement.ReferenceHandlingPolicy;
...
/*        if (rest.getOperation().size() > 0) {
  ...
  private void addSupportedFormats(XhtmlNode uList, CapabilityStatement conf) {
    ...
    Boolean first = true;
    String capExpectation = null;
    for (Enumeration<org.hl7.fhir.r6.model.CapabilityStatement.SupplementedMimeTypes> c : conf.getFormat()) {
      if (!first) {
        lItem.addText(", ");
      }
      capExpectation = getExtValueCode(c.getExtensionByUrl(EXPECTATION));
    ...
    if (conf.hasPatchFormat()) {
      ...
      first=true;
      for (Enumeration<org.hl7.fhir.r6.model.CapabilityStatement.PatchMimeTypes> c : conf.getPatchFormat()) {
        if (!first) {
          lItem.addText(", ");
        }
        capExpectation = getExtValueCode(c.getExtensionByUrl(EXPECTATION));
