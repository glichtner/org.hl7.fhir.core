    ...
    for (File f : src.listFiles()) {
      if (f.isDirectory()) {
        ...
        if (j.has("resourceType")) {
          ...
          try {
            var res = new org.hl7.fhir.r6.formats.JsonParser().parse(FileUtilities.fileToBytes(tgt));
            new org.hl7.fhir.r6.formats.JsonParser().setOutputStyle(OutputStyle.PRETTY).compose(new FileOutputStream(tgt), res);
          } catch (Exception e) {
