...
import org.hl7.fhir.r6.model.QuestionnaireResponse;
import org.hl7.fhir.r6.model.QuestionnaireResponse.QuestionnaireResponseItemAnswerComponent;
import org.hl7.fhir.r6.model.QuestionnaireResponse.QuestionnaireResponseItemComponent;
...
//    case DEFNS: return renderDefns(x, q);
  ...
  public void genDefinitionLink(HierarchicalTableGenerator gen, QuestionnaireResponseItemComponent i, Cell defn, Resource src) {
    String path = null;
    String dOrig = i.hasDefinition() ? i.getDefinition().get(0).getValue() : null;
    if (dOrig == null) return;
    String d = dOrig;
    if (d.contains("#")) {
      path = d.substring(d.indexOf("#")+1);
      d = d.substring(0, d.indexOf("#"));
    }
    StructureDefinition sd = context.getWorker().fetchResource(StructureDefinition.class, d, ExtensionUtilities.getVersionResolutionRules((i.hasDefinition() ? i.getDefinition().get(0) : null)), null, src);
    if (sd != null) {
      String url = sd.getWebPath();
      if (url != null) {
        defn.getPieces().add(gen.new Piece(url+"#"+path, path, null));
      } else {
        defn.getPieces().add(gen.new Piece(null, dOrig, null));
      }
    } else {
      defn.getPieces().add(gen.new Piece(null, dOrig, null));
    }
